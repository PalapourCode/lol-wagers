...
export default async function handler(req, res) {
  ...
  const { action, username, lolAccount, puuid, rank } = req.body || req.query;

  try {
    if (action === "linkAccount") {
      // Check if puuid already linked to another user
      ...
      await sql`UPDATE users SET rank = ${rank} WHERE username = ${username}`;
